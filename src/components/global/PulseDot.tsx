...
const PulseDot = ({
  ...
  return (
    <span className={`relative flex ${sizeClass}`}>
      <span
        className={`absolute inline-flex ${sizeClass} rounded-full ${appliedClass || "bg-greenText"} ${animate ? "animate-ping" : ""} opacity-75`}
        style={appliedStyle}
      />
      <span
        className={`relative inline-flex ${sizeClass} rounded-full ${appliedClass || "bg-greenText"}`}
        style={appliedStyle}
      />
    </span>
  );
