...
interface UseAdaPriceWithTxCount {
  json: any;
...
export const useAdaPriceWithTxCount = (
  ...
  const allMergedData = useMemo(() => {
    const analyticsMap = new Map(
      analyticsData.map(item => [item.date, item.stat?.count_tx ?? 0]),
    );
    ...
    return graphRates
      ...
      .map(rate => ({
        ...
        adausd: rate.adausd,
        btcusd: rate.btcusd,
        txCount: analyticsMap.get(rate.date) ?? 0,
      }));
  ...
  const txCounts = displayData.map(d => d.txCount);
