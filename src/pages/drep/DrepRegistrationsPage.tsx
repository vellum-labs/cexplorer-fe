...
import { useFetchDrepRegistrations } from "@/services/tx";
...
export const DrepRegistrationsPage = () => {
  const { t } = useAppTranslation("common");
  const [totalItems, setTotalItems] = useState(0);
  const { page } = useSearch({ from: "/drep/registrations" });
  const {
    columnsVisibility,
    columnsOrder,
    setColumsOrder,
  ...
  const query = useFetchDrepRegistrations(rows, (page ?? 1) * rows - rows);
  ...
  return (
    <PageBase
      ...
      title={t("drep.registrations.title")}
      breadcrumbItems={[
        {
        ...
        { label: t("drep.registrations.breadcrumb") },
      ]}
    >
        ...
        <div className='mb-2 flex w-full items-center justify-between gap-1'>
          {!totalItems ? (
            ...
            <h3 className='basis-[230px]'>
              {t("drep.registrations.total", { count: formatNumber(totalItems ?? 0) })}
            </h3>
          )}
